...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/03/28 11:10:16 by anyo              #+#    #+#             */
/*   Updated: 2018/04/06 16:13:26 by anyo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	h_fill_loop(char **heredocs, char *delimiter, int x)
{
	...
	if ((ret = input_read(&tmp1, PROMPT_QUOTE)) == EXIT_FAILURE)
		return (EXIT_FAILURE);
...
static int	h_fill(char **heredocs, char *delimiter, int x)
{
	...
	sh_prompt(PROMPT_QUOTE);
	if ((ret = input_read(&heredocs[x], PROMPT_QUOTE)) == EXIT_FAILURE)
		return (EXIT_FAILURE);
	else if (ret == EXIT_CTRL_C)
	{
		ft_strdel(&delimiter);
	...
	if (ft_strcmp(heredocs[x], delimiter) == 0)
	{
